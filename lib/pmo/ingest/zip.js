...
//
// The compression itself comes from `codec.js`, which the browser build swaps
// for a version that needs no Node.
import { deflateRaw, inflateRaw } from './codec.js';
...
export function unzip(buffer) {
  ...
  for (let i = 0; i < entryCount; i += 1) {
    ...
    else if (method === 8) files.set(name, Buffer.from(inflateRaw(raw)));
...
export function zip(entries, { modified = new Date(2020, 0, 1, 0, 0, 0) } = {}) {
  ...
  for (const [name, contents] of entries) {
    ...
    const deflated = Buffer.from(deflateRaw(data));
